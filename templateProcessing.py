...
class ImageProcessingManager(metaclass=Singleton):
    ...
    def __init__(self, image_path="input_image/ronaros1.png", templates=None):
        ...
        if templates is None:  # Default go-to template.
            self._templates = [
                Template(
                    image_path="templates/main/aa_temp.png",
                    label="1",
                    color=(125, 0, 255),
                ),
            ]
    ...
    def processImage(self):
        ...
        try:
            ...
            image_with_detections = (
                self._image.copy()
            )  # Copy of the main image to work on
            ...
            for detection in self._detections:
                ...
                detection_img = image_with_detections[
                    detection["TOP_LEFT_Y"] : detection["BOTTOM_RIGHT_Y"],
                    detection["TOP_LEFT_X"] : detection["BOTTOM_RIGHT_X"],
                ]
            ...
            self.findItemSquares(
                image_with_detections,
                15,
                self._detections[0]["TOP_LEFT_X"],
                self._detections[0]["BOTTOM_RIGHT_Y"],
            )
    ...
    def findItemSquares(
        self, image_with_detections, amount, init_top_left_x, init_top_left_y
    ):
        ...
        for i in range(amount):  # TODO: add exceptions to certain items.
            if (
                ...
                y = (
                    init_top_left_y
                    + INITIAL_Y_PADDING
                    + BDO_ITEM_SQUARE_HEIGHT
                    + BDO_ITEM_SQUARE_PADDING * 2
                    + 1
                )
            square_detection = {
                "TOP_LEFT_X": x,
            ...
            self._square_img.append(
                image_with_detections[
                    y : y + BDO_ITEM_SQUARE_HEIGHT, x : x + BDO_ITEM_SQUARE_WIDTH
                ]
            )
    ...
    def matchItemToTemplates(self):
        ...
        templates = FileManager.get_files(
            f"templates/{self.loot_table_name}", ["png", "jpeg", "jpg"]
        )
        print(templates)
        loot_table = [
            Template(tmp, FileManager.get_filename_from_path(tmp), (125, 125, 125))
        ...
        for image in self._square_img:
            ...
            for template in loot_table:
                percentage = (
                    self.calculateImageResemblance(image, template.template) * 100
                )
    ...
    def calculateImageResemblance(self, image1, image2):
        ...
        return ssim(
            cv2.resize(image1, (50, 50)),
            cv2.resize(image2, (50, 50)),
            multichannel=True,
        )
